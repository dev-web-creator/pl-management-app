...
- 🚧 過去FY（FY-1〜FY-3）データのインポート（ADR-045）：**ローカル取込・全月突合NG0件まで完了**（取引2,211件・給与明細25ヶ月・残高157件・目標18件）。
  残タスク＝本番Neonへの投入（`data/import-sheet-fy1-fy3.sql` を Neon SQL Editor で実行。**事前にアプリ実データ開始月を確認して重複月を除外**）
