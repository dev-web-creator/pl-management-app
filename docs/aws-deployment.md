...
✅ **自動化済み（GitHub Actions）**: master へ push すると build: 
build status: 
service status: 
service is  — image updated only (deploys on next resume + start-deployment) が
S3→CodeBuild→App Runner を自動実行する（OIDC認証・キーレス）。App Runner が停止中なら
イメージ更新のみ行い、次回起動時に最新を反映する。以下は手動で行う場合のコマンド（参考）：
