...
## ✅ できていること（実装済み）
- ✅ DB：スキーマ14テーブル＋初期データ（ローカル `pl_app` / 本番 Neon 両方）
...
- ✅ 二重入力防止（冪等キー `client_key`・ADR-040）：連打・通信リトライ・再送信でも取引は1件のみ（DB部分ユニーク索引＋API＋フォームの3層）
- ✅ 分析ページ（`/analytics`・ADR-041）：今月vs直近平均カード・12ヶ月の収入/支出/黒字チャート（インラインSVG）・カテゴリ前月比
- ✅ 通知基盤（ADR-042）：変動費しきい値（既定10/15/20/25/30万円）到達でログインメールへ通知。ルールは `/settings` で追加/ON・OFF/削除（月1回だけ・多重送信ガード付き）。**送信有効化は `RESEND_API_KEY` 設定**
- ✅ 設定画面（`/settings` ⚙️）：FY開始月の変更UI（ADR-017の残タスク解消）＋通知ルール管理＋送信履歴
- ✅ 機能の表示ON/OFF（ADR-046）：使わないページをユーザーごとにナビから隠せる（設定画面の🧩パネル。サマリ・設定は固定）
- ✅ 月次ページ（`/monthly` 🧾・ADR-047）：現運用の月次支出/収入タブを再現（収入行その場記録・固定費予実・変動費階層・手取り比・月確定）。固定費マスタ管理は設定へ移設。過去月は実績のみでシートと1円一致
- ✅ 口座・カード・電子マネーのマスタ管理（`/wallets`・ADR-048）：全種別CRUD・クレカ締め日/支払日/引落先・無効化/復活。新規ユーザーが自分の決済手段を登録可
- ✅ 費目（カテゴリ）のマスタ管理（`/categories`・ADR-050）：ツリー追加/改名/無効化/削除・任意階層のネスト
- ✅ 給与明細→月次「給与収入(手取り)」自動連動（ADR-049）：手取りが収入と口座残高に反映（冪等）
- ✅ 使い方ガイド（`/guide`）＋新規ユーザーのオンボーディング（ADR-051）
- ✅ セキュリティ：/inspect をオーナー専用にゲート（マルチユーザーの情報漏洩修正・ADR-052）
- ✅ データエクスポート（CSV/JSON・ADR-053）：設定から取引CSV・全データJSONを書き出し
- ✅ 利用規約・プライバシーポリシー草案（`/legal/*`・ADR-053）：外部送信を開示。※公開前に専門家レビュー
- ✅ セルフサインアップ（ADR-053）：`AUTH_OPEN_SIGNUP=true` で誰でも登録可（未設定は招待制）
- ⬜ 課金（Stripe等）：料金モデル・Stripeアカウントの意思決定が必要（実装コードは無料・決済手数料は別）

## 🚧 着手中・直近の宿題
- ✅ **本番の認証有効化**（ADR-037）：Vercel / App Runner とも Google ログイン有効化済み。`AUTH_ALLOWED_EMAILS` で招待制運用中（2026-07-19確認）。
- 🚧 **給与明細OCRの有効化**（ADR-039）：コード完成・**PDF対応＋モデル自動フォールバック追加（2026-07-20）**。`GEMINI_API_KEY` 設定済み・本番での読み取り成功確認待ち。
- 🚧 **メール通知の有効化**（ADR-042）：コード完成。Vercel/App Runner に `RESEND_API_KEY`（resend.com 無料枠100通/日）を設定すると送信開始。
...
- ✅ 暗号資産の資産管理（ADR-043）：wallet type='crypto'＋評価額の手入力（最新スナップショット＝正）。`/assets` の₿パネルで銘柄追加・評価額更新→総資産/内訳/推移に自動反映。価格API非依存
- ✅ 5か年PL・将来月フォーキャスト（`/forecast` 🔮・ADR-044）：実績＋targets目標＋自動見込み（固定費マスタ＋直近6ヶ月平均）で60ヶ月。純資産予測ライン付き
- 🚧 過去FY（FY-1〜FY-3）データのインポート（ADR-045）：**ローカル取込・全月突合NG0件まで完了**（全期間・取引2,301件・給与明細25ヶ月・残高157件・目標18件）。
  残タスク＝本番Neonへの投入（ユーザーが `data/import-sheet-fy1-fy3.sql` を Neon SQL Editor で実行）。
  並走運用（シートが正）：シート更新→xlsx再共有→`db/import/gen_import.py` 再生成→再実行で差分のみ追加（冪等）
...
- ✅ 給与明細OCR（ADR-039）：実装済み。**Vercelに `GEMINI_API_KEY`（Google AI Studio無料）を設定すると有効化**（設定までボタン非表示）。
...
- ✅ FY開始月の設定UI（ADR-017 → `/settings` に実装・ADR-042）
- ✅ `card_statements` テーブルの扱い → **温存で確定**（2026-07-19。現方式で問題なし・撤去は破壊的変更のためリスクのみ）

## 今やること（直近の順番）
1. ✅ 取引一覧＋削除（P1）
2. ✅ 固定費「実額で記録」ワンタップ（ADR-030）
3. ✅ /inspect のルート単位保護（本番キー必須）
4. ✅ サイト全体の認証（ADR-032→037でGoogleログインとして実現。Vercel/AWSとも有効化済み・招待制）
5. ✅ P2（振替/カード/分割/予実）・P3の大半（給与/資産/リコンサイル/分析ほか）完了
6. 残り：GEMINI_API_KEY設定（OCR有効化）・RESEND_API_KEY設定（メール通知有効化）→ 暗号資産/5か年PL/過去FYインポートは着手要否を判断
